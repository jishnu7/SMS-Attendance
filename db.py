...
class database():
        ...
        for line in f.readlines():
            if line.split(' DELETE ')[0] == \
                ...
                try:
                    ...
                    print "RESTORE DELETED ACCOUNTS " \
                          + account + '--' + password + '--' + mob
                except MySQLdb.Error, e:
                    print "Error %d: %s" % (e.args[0], e.args[1])
                    continue
        # Empty the file
    ...
    def fetch(self, start, limit):
        ...
        while True:
            cursor = self.db.cursor()
            query = "SELECT `username`, `passwd`, `sem`, `mob` " \
                    "FROM `users` WHERE `disabled`=0 ORDER BY `id` ASC "
            if limit != 0:
    ...
    def update(self, username, date):
        ...
        cursor = self.db.cursor()
        query = "UPDATE `users` SET `last_update`='" + date + "' "\
